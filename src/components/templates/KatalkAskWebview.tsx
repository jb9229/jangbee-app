        ...
        >
          <Contents>
            <WebView
              ...
              originWhitelist={['*']}
              onShouldStartLoadWithRequest={(e): boolean =>
...
const handleShouldStartLoadWithRequest = (evt: any): boolean => {
  if (
    evt.url.startsWith('http://') ||
    evt.url.startsWith('https://') ||
    evt.url.startsWith('about:blank')
  ) {
    return true;
  }
  ...
  if (Platform.OS === 'android') {
    ...
    Linking.openURL(evt.url).catch(err => {
      noticeUserError(
        'KatalAskWebview[Linking.openURL]',
        `evt.url is: ${err.message}`
      );
    });
  }
