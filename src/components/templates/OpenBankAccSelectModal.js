    ...
    if (mode === ACCOUNT_MODE) {
      ...
      if (!couponSelected) {
        if (!selFinUseNum) {
          notifyError(
            '유효성검사 에러',
            `쿠폰 또는 계좌번호를[${selFinUseNum}] 선택해 주세요`
          );
          return;
        }
      }
      ...
      if (!v[0]) {
        notifyError(
          '유효성검사 에러',
          `캐쉬백 요청값이 올바르지 않습니다(1회 최고 요청가능금액: 20만원): [${requestCashback}]`
        );
        return;
      }
          ...
          <View style={styles.bgWrap}>
            <View style={styles.contentsWrap}>
              ...
              {mode === CASHBACK_MODE ? (
                <CashbackContainer>
                  ...
                  <JBTextInput
                    ...
                    onChangeText={text =>
                      this.setState({ requestCashback: text })
                    }
              ...
              <FlatList
                ...
                renderItem={({ item }) =>
                  OBAccount(item, selFinUseNum, this.onAccListItemPress)
                }
