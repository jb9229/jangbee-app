...
export default class WorkItem extends React.PureComponent {
  render() {
    ...
    return (
      <Card
        Finished={item.workState === 'CLOSED' && item.firmEstimated}
        bgColor={cardColor}
      >
        <TagWrap>
          {item.modelYearLimit ? (
            <JBTag name={`${item.modelYearLimit}년식이상`} />
          ) : null}
          {item.licenseLimit ? (
            <JBTag name={`${item.licenseLimit}필요`} />
          ) : null}
          {item.nondestLimit ? (
            <JBTag name={`비파괴검사 ${item.nondestLimit}개월이하`} />
          ) : null}
          {item.careerLimit ? (
            <JBTag name={`${item.careerLimit}년경력이상`} />
          ) : null}
        </TagWrap>
        <JBTextItem
          title="장비"
        ...
        <JBTextItem
          title="기간"
          value={`${item.startDate} ~ ${item.endDate}(${convertWorkStr(
            item.period
          )})`}
          small
          titleSize={70}
        ...
        {hideAddress ? (
          <JBTextItem
            title="현장주소"
            value={`${item.sidoAddr} ${item.sigunguAddr}`}
            small
          />
        ) : (
          <JBTextItem
