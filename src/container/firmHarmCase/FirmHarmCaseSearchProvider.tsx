...
import {
  CallHistory,
  MY_FIRMHARMCASE_SEARCHWORD,
  TOTAL_FIRMHARMCASE_SEARCHWORD,
} from './type';
...
const FirmHarmCaseSearchProvider = (props: Props): React.ReactElement => {
  ...
  const [searchQueryVariables] = React.useState({
    firmCaseListInput: {
      accountId: '',
      searchWord: '',
      pageQueryInfo: { first: PAGING_COUNT },
    },
  });
  ...
  const [deleteRequest, deleteResponse] = useMutation(FIRMHARMCASE_DELETE, {
    ...
    onError: err => {
      noticeUserError(
        'FirmHarmCaseSearchProvider(deleteRequest -> error)',
        err?.message,
        user
      );
    },
  ...
  console.log(
    '>>> firHarmCasesRsp: ',
    firHarmCasesRsp?.data?.firmHarmCases?.edges
  );
  ...
  React.useEffect(() => {
    ...
    if (props.initSearchAll) {
      ...
      searchQueryVariables.firmCaseListInput.accountId = userProfile.uid;
      firHarmCasesReq({ variables: searchQueryVariables });
      setSearched(true);
  ...
  const actions = {
    ...
    onEndReachedCaseList(): void {
      if (!firHarmCasesRsp?.data?.firmHarmCases?.pageInfo?.hasNextPage) {
        return;
      }

      firHarmCasesRsp.fetchMore({
        variables: {
          cursor: firHarmCasesRsp.data.firmHarmCases.pageInfo.endCursor,
        },
