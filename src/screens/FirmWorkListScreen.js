...
class FirmWorkListScreen extends React.Component
{
  ...
  init = async () =>
  {
    ...
    {
      this.setState({ myEquipment: firmInfo.equipment }, () =>
        this.setOpenWorkListData()
      );
    }
    else
  ...
  cancelDispatchFee = fintechUseNum =>
  {
    ...
    api
      ...
      .then(res =>
      {
        ...
        Alert.alert(
          '배차비 환불 문제',
          `네트워크 환경 확인 후 다시 시도해 주세요(${res.rsp_code}, ${
            res.rsp_message
          })`,
          [
            {
              text: '환불 다시 시도하기',
              onPress: () => this.cancelDispatchFee(fintechUseNum)
            },
            { text: '취소', onPress: () => {} }
          ],
          { cancelable: false }
      ...
      .catch(error =>
      {
        notifyError(
          '배차비 환불 문제',
          `네트워크 환경 확인 후 다시 시도해 주세요(${error.name}, ${
            error.message
          })`,
          [
            {
  ...
  confirmApplyFirmWork = work =>
  {
    ...
    if (
      firmInfo &&
      firmInfo.modelYear &&
      Number(firmInfo.modelYear) < Number(work.modelYearLimit)
    )
    {
      Alert.alert(
        ...
        `${
          work.modelYearLimit
        }년식이상을 요청한 일감으로 지원할 수 없습니다(보유장비: ${
          firmInfo.modelYear
        }년식)`
      );
    ...
    Alert.alert(
      ...
      [
        { text: '취소', onPress: () => {} },
        {
          ...
          onPress: () =>
            this.setState({
              isVisibleAccSelModal: true,
              acceptWorkId: work.id,
              withdrawCompleteFnc: this.applyFirmWork
            })
        }
      ]
    );
        ...
        { text: '포기하기', onPress: () => this.abandonWork(workId) },
        {
          ...
          onPress: () =>
            this.setState({
              isVisibleAccSelModal: true,
              acceptWorkId: workId,
              withdrawCompleteFnc: this.withdrawDispatchFee
            })
        }
      ],
      { cancelabel: false }
  ...
  withdrawDispatchFee = (couponSelected, fintechUseNum) =>
  {
    ...
    api
      ...
      .then(res =>
      {
        ...
        notifyError(
          '배차비 출금 문제',
          `네트워크 환경확인 또는 통장잔액을 확인후 다시 시도해 주세요(${
            res.rsp_code
          }, ${res.rsp_message})`
        );
      })
  ...
  render ()
  {
    ...
    const renderOpenWorkList = () => (
      <FirmOpenWorkList
        list={openWorkList}
        handleRefresh={() =>
          this.setState({ openWorkListRefreshing: true }, () =>
            this.setOpenWorkListData()
          )
        }
    ...
    return (
      <View style={styles.Container}>
        ...
        <JBButton
          title="차주 일감 등록하기"
          onPress={() =>
            navigation.navigate('WorkRegister', { firmRegister: true })
          }
          size="full"
          Primary
        />
        <ModalView>
          <OpenBankAccSelectModal
            ...
            completeSelect={(fintechUseNum, couponSelected) =>
            {
              withdrawCompleteFnc(couponSelected, fintechUseNum);
            }}
            accountId={user.uid}
...
export default FirmWorkListScreen;
