...
const sidoEquiPickerItems = ['크레인', '카고크레인', '굴착기', '스카이'].map(lin => (
...
export default class FirmCntChart extends React.Component {
    ...
    api
      ...
      .then((firmCNTData) => {
        ...
        if (firmCNTData) {
          ...
          this.setState({ firmCntChartData: chartData, firmCntChartModels: modelList });
        } else {
          Alert.alert('유효하지 않은 차트데이터 입니다', `응답 내용: ${firmCNTData}`);
        }
      })
      .catch((error) => {
        if (!this._isMounted) {
  ...
  render() {
    const { firmCntChartData, selectedChartEqui, firmCntChartModels } = this.state;
    ...
    return (
      <ChartWrap>
        ...
        {firmCntChartData && (
          <ScrollView horizontal>
            <LineChart data={firmCntChartData} width={900} height={220} chartConfig={chartConfig} />
          </ScrollView>
        )}
      </ChartWrap>
    );
